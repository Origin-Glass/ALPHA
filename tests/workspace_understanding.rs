...
async fn finish_run(pool: &PgPool, id: Uuid, stdout: &str) {
    let image = "alpha-judge-runner@sha256:e0a1147badcf2997c64f1cc3058d015ea0cf6865511d09e2f1506f06377d89de";
    ...
    assert!(
        alpha::workspaces::complete_run(pool, id, lease.lease_token, &outcome)
            .await
            .unwrap()
    )
}
...
async fn owned_workspace_run_understanding_and_portfolio_receipts_are_exact(pool: PgPool) {
    ...
    let created=post(&app,&owner,"/api/v1/workspaces",json!({"title":"다중 파일 CLI","template_slug":"rust-cli-v1","project_id":null,"files":files("41"),"idempotency_key":Uuid::now_v7()})).await;
    ...
    let same=body(post(&app,&owner,"/api/v1/workspaces",json!({"title":"같은 템플릿","template_slug":"rust-cli-v1","project_id":null,"files":files("99"),"idempotency_key":Uuid::now_v7()})).await).await;
    ...
    assert_eq!(post(&app,&owner,&submit_uri,json!({"explanation":"main.rs에서 helper.rs 값을 읽고 상태 전이 뒤 오류 처리를 유지합니다","prediction":"999","modification":"주석만 변경했습니다","transfer_answer":"44","modification_run_id":bad_mod,"transfer_run_id":runs[2],"idempotency_key":Uuid::now_v7()})).await.status(),StatusCode::BAD_REQUEST);
    assert_eq!(post(&app,&owner,&submit_uri,json!({"explanation":"main.rs에서 helper.rs 값을 읽고 상태 전이 뒤 오류 처리를 유지합니다","prediction":"wrong","modification":"helper 값을 독립 변경했습니다","transfer_answer":"wrong","modification_run_id":runs[1],"transfer_run_id":runs[2],"idempotency_key":Uuid::now_v7()})).await.status(),StatusCode::BAD_REQUEST);
    let submit=post(&app,&owner,&submit_uri,json!({"explanation":"main.rs에서 helper.rs 값을 읽고 상태 전이 뒤 오류 처리를 유지합니다","prediction":"43","modification":"helper 값을 독립 변경했습니다","transfer_answer":"44","modification_run_id":runs[1],"transfer_run_id":runs[2],"idempotency_key":Uuid::now_v7()})).await;
    ...
    sqlx::query("INSERT INTO user_roles(user_id,role) VALUES($1,'RIGHTS_REVIEWER')")
        ...
        .await
        .unwrap();
    let rights=post(&app,&other,"/api/v1/portfolio/rights",json!({"source_run_id":runs[2],"provenance":"original","license_identifier":"Copyright-owner","idempotency_key":Uuid::now_v7()})).await;
    assert_eq!(rights.status(), StatusCode::CREATED);
    let rights = body(rights).await["id"].clone();
    assert_eq!(post(&app,&owner,"/api/v1/portfolio",json!({"title":"과장","problem":"학습 기록 계산","target_user":"학습자","workspace_id":transfer_id,"source_run_id":runs[2],"rights_receipt_id":rights,"visibility":"public","assistance_disclosure":"문서 탐색 도움 사용","evidence_labels":["BUILT","TESTED"],"idempotency_key":Uuid::now_v7()})).await.status(),StatusCode::BAD_REQUEST);
    let draft=post(&app,&owner,"/api/v1/portfolio",json!({"title":"학습 CLI","problem":"학습 기록 계산","target_user":"학습자","workspace_id":transfer_id,"source_run_id":runs[2],"rights_receipt_id":rights,"visibility":"public","assistance_disclosure":"문서 탐색 도움 사용","evidence_labels":["BUILT","EXPLAINED","INDEPENDENTLY_MODIFIED","TRANSFER_VERIFIED"],"idempotency_key":Uuid::now_v7()})).await;
...
async fn workspace_lease_cancel_retry_and_stale_completion_are_fenced(pool: PgPool) {
    ...
    let created=body(post(&app,&owner,"/api/v1/workspaces",json!({"title":"임대 검증","template_slug":"rust-cli-v1","project_id":null,"files":files("42"),"idempotency_key":Uuid::now_v7()})).await).await;
    ...
    assert!(
        !alpha::workspaces::complete_run(&pool, id, first.lease_token, &outcome)
            .await
            .unwrap()
    );
    assert_eq!(
    ...
    assert!(
        !alpha::workspaces::complete_run(&pool, id, second.lease_token, &outcome)
            .await
            .unwrap()
    );
...
async fn forged_success_and_self_rights_approval_do_not_create_evidence(pool: PgPool) {
    ...
    let workspace=body(post(&app,&owner,"/api/v1/workspaces",json!({"title":"위조 방지","template_slug":"rust-cli-v1","project_id":null,"files":files("42"),"idempotency_key":Uuid::now_v7()})).await).await;
    let run = body(
        post(
            &app,
            &owner,
    ...
    sqlx::query("UPDATE workspace_runs SET status='succeeded',exit_code=0,stdout='42',stdout_hash=$2,completed_at=now() WHERE id=$1")
        .bind(run_id).bind(vec![9_u8;32]).execute(&pool).await.unwrap();
...
async fn revisions_checkpoints_reset_and_expiry_are_owned_and_fenced(pool: PgPool) {
    ...
    let make = |title: &str| json!({"title":title,"template_slug":"rust-cli-v1","project_id":null,"files":files("41"),"idempotency_key":Uuid::now_v7()});
    ...
    let a = post(
        ...
        &owner,
        &uri,
        json!({"name":"안정 버전","idempotency_key":checkpoint_key}),
    )
    .await;
    ...
    assert!(
        !alpha::workspaces::complete_run(&pool, run_id, lease.lease_token, &outcome)
            .await
            .unwrap()
    );
