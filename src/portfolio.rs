...
pub async fn create(
    ...
    if req.title.trim().is_empty()
        ...
        || !valid_evidence_labels(&req.evidence_labels)
    {
    ...
    let request_hash=Sha256::digest(serde_json::to_vec(&json!({"title":req.title.trim(),"problem":req.problem.trim(),"target_user":req.target_user.trim(),"workspace_id":req.workspace_id,"source_run_id":req.source_run_id,"rights_receipt_id":req.rights_receipt_id,"visibility":req.visibility,"assistance_disclosure":req.assistance_disclosure.trim(),"evidence_labels":req.evidence_labels})).map_err(|_|PortfolioError::Invalid("포트폴리오 요청을 처리할 수 없습니다"))?).to_vec();
    ...
    let exact:Option<(bool,bool,bool)>=sqlx::query_as("SELECT r.status='succeeded' AND r.deterministic_checks_passed AND r.runner_receipt_hash IS NOT NULL AND rights.workspace_version=r.workspace_version AND rights.artifact_hash=r.artifact_hash,EXISTS(SELECT 1 FROM understanding_receipts u WHERE u.user_id=r.user_id AND u.transfer_run_id=r.id AND u.state='transfer_verified' AND u.deterministic_checks_passed),t.supports_tests FROM workspace_runs r JOIN workspace_templates t ON(t.id,t.revision,t.template_digest)=(r.template_id,r.template_revision,r.template_digest) JOIN portfolio_rights_receipts rights ON(rights.workspace_id,rights.user_id)=(r.workspace_id,r.user_id) WHERE r.id=$1 AND r.user_id=$2 AND r.workspace_id=$3 AND rights.id=$4 AND rights.decision='approved'").bind(req.source_run_id).bind(user).bind(req.workspace_id).bind(req.rights_receipt_id).fetch_optional(&mut *tx).await?;
    let (run_verified, understanding_verified, supports_tests) = exact.ok_or(
        PortfolioError::Invalid("정확한 산출물 리비전의 권리 승인 영수증이 필요합니다"),
    )?;
    ...
    {
        return Err(PortfolioError::Invalid(
            "실행·테스트·이해 영수증으로 확인되지 않은 증거 라벨을 주장할 수 없습니다",
        ));
    }
...
pub async fn approve_rights(
    ...
    let run:Option<(Uuid,Uuid,i32,Vec<u8>)>=sqlx::query_as("SELECT workspace_id,user_id,workspace_version,artifact_hash FROM workspace_runs WHERE id=$1 AND status='succeeded' AND deterministic_checks_passed AND runner_receipt_hash IS NOT NULL FOR UPDATE").bind(req.source_run_id).fetch_optional(&mut *tx).await?;
    let (workspace, user, version, artifact) = run.ok_or(PortfolioError::NotFound)?;
    if user == reviewer {
        return Err(PortfolioError::Conflict(
