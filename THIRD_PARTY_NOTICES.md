...
Rust 허용 목록은 `deny.toml`의 Apache-2.0, BSD-3-Clause, CDLA-Permissive-2.0, ISC, MIT, Unicode-3.0, Zlib입니다. Web lockfile 허용 목록은 0BSD, Apache-2.0, BSD-2-Clause, BSD-3-Clause, BlueOak-1.0.0, CC0-1.0, ISC, MIT, MIT-0, MPL-2.0입니다. MPL-2.0 구성요소를 수정·배포하면 해당 파일 수준 source 제공 의무를 별도 검토합니다. GPL/AGPL 신규 의존성은 dependency review에서 차단합니다.
...
Dockerfile과 운영 Compose의 외부 이미지는 digest로 고정됩니다. 이미지 배포 시 base image 안의 `/usr/share/doc`, image label 또는 upstream notice를 제거하지 말고, registry가 제공하는 SBOM/attestation과 해당 release의 license 자료를 함께 보관합니다.
...
CI는 고정 commit의 GitHub Actions, 고정 버전 감사 도구, `Cargo.lock`, `package-lock.json` integrity, gitleaks digest, 모든 Dockerfile/운영 image digest를 검사합니다. 의존성을 추가하거나 올릴 때 이 고지의 구성요소와 허용 정책도 함께 갱신합니다.
