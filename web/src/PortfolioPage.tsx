...
type Portfolio = { id: string; title: string; problem: string; target_user: string; visibility: string; assistance_disclosure: string; evidence_labels: string[]; publishable: boolean; published: boolean };
...
function PortfolioPage() {
  ...
  const [form, setForm] = useState({ title: '', problem: '', target_user: '', workspace_id: '', assistance_disclosure: '' });
  ...
  useEffect(() => { fetch('/api/portfolio', { credentials: 'include' }).then((response) => response.json()).then((body) => setItems((current) => [...current, ...(body.items ?? []).filter((item: Portfolio) => !current.some((candidate) => candidate.id === item.id))])).catch(() => setMessage('포트폴리오를 불러오지 못했습니다.')); }, []);
  const update = (field: keyof typeof form, value: string) => setForm((current) => ({ ...current, [field]: value }));
  const create = async () => {
    const payload = { ...form, visibility: 'private' };
    const response = await fetch('/api/portfolio', { method: 'POST', credentials: 'include', headers, body: JSON.stringify({ ...payload, idempotency_key: keyFor('create', payload) }) });
  ...
  const publish = async (item: Portfolio) => {
    const response = await fetch(`/api/portfolio/${item.id}/publish`, { method: 'POST', credentials: 'include', headers, body: JSON.stringify({ idempotency_key: keyFor('publish', item.id) }) });
    const body = await response.json();
  ...
  return <div className="learning-page"><PortalHeader /><main className="path-panel portfolio-panel">
    ...
    <section className="project-form" aria-label="포트폴리오 만들기">
      ...
      <button type="button" disabled={!form.title.trim() || !form.problem.trim() || !form.target_user.trim() || !form.assistance_disclosure.trim()} onClick={create}>비공개 포트폴리오 만들기</button>
    </section>
    <div className="portfolio-grid">{items.map((item) => <article className="evidence-card" key={item.id}><h2>{item.title}</h2><p>{item.problem}</p><p>대상: {item.target_user}</p><p>공개 범위: {item.visibility === 'public' ? '공개' : '비공개'}</p><p>도움 공개: {item.assistance_disclosure}</p><ul className="evidence-list">{item.evidence_labels.map((label) => <li key={label}>{evidenceLabels[label] ?? label}</li>)}</ul>{item.publishable && !item.published && <button type="button" onClick={() => publish(item)}>권리 확인 후 게시</button>}</article>)}</div>
