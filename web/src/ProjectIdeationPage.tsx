...
function ProjectIdeationPage() {
  ...
  return <div className="learning-page"><PortalHeader /><main className="path-panel project-learning-panel"><p className="eyebrow">프로젝트 아이디어 스튜디오</p><h1>관심을 작은 결과로</h1>
    <form className="project-form" onSubmit={submit}><label>프로젝트 이름<input value={title} maxLength={120} onChange={(e) => { setTitle(e.target.value); ideaKey.current = crypto.randomUUID(); }} required /></label><label>원하는 기능(쉼표로 구분)<textarea value={features} maxLength={1000} onChange={(e) => { setFeatures(e.target.value); ideaKey.current = crypto.randomUUID(); }} required /></label><button className="primary-action" type="submit">실행 가능한 범위 확인</button></form>
