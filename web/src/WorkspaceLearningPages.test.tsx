...
test('여러 파일을 저장하고 격리 실행을 취소하며 같은 저장 재시도에는 같은 멱등성 키를 쓴다', async () => {
  ...
  const fetchMock = vi.fn().mockImplementation((path: string, options?: RequestInit) => {
    if (path === '/api/workspaces/w1' && !options?.method) return ok({ id: 'w1', title: '한국어 기록 CLI', files: [{ path: 'src/main.rs', content: 'fn main() {}' }, { path: 'README.md', content: '# 기록' }] });
    if (path === '/api/workspaces/w1/files') return ok({ saved: true });
    if (path === '/api/workspaces/w1/runs') return ok({ id: 'run-1', status: 'running' });
    if (path === '/api/workspace-runs/run-1') return ok({ id: 'run-1', status: 'running', stdout: '', stderr: '' });
    if (path === '/api/workspace-runs/run-1/cancel') return ok({ id: 'run-1', status: 'cancelled' });
  ...
  fireEvent.click(screen.getByRole('button', { name: '파일 저장' }));
  await waitFor(() => expect(fetchMock.mock.calls.filter(([path]) => path === '/api/workspaces/w1/files')).toHaveLength(2));
  const saves = fetchMock.mock.calls.filter(([path]) => path === '/api/workspaces/w1/files');
  expect(JSON.parse(saves[0][1].body).idempotency_key).toBe(JSON.parse(saves[1][1].body).idempotency_key);
...
test('설명·변형·전이 답변을 제출하고 서버 증거만 표시하며 MASTERED로 과장하지 않는다', async () => {
  ...
  const fetchMock = vi.fn().mockImplementation((path: string) => {
    if (path === '/api/understanding/challenges') return ok({ id: 'c1', prompt: '입력 흐름을 설명하고 다른 자료형에 적용하세요.', state: 'REVIEWED' });
    if (path === '/api/understanding/challenges/c1/submit') return ok({ state: 'TRANSFER_VERIFIED', evidence_labels: ['BUILT', 'TESTED', 'EXPLAINED', 'INDEPENDENTLY_MODIFIED', 'TRANSFER_VERIFIED'], assistance_disclosure: '공식 문서만 사용' });
    throw new Error(`unexpected ${path}`);
  });
  vi.stubGlobal('fetch', fetchMock);
...
test('포트폴리오는 비공개로 만들고 도움 공개와 검증된 증거 라벨을 게시 전 확인한다', async () => {
  ...
  const item = { id: 'p1', title: '학습 기록 CLI', problem: '기록이 흩어짐', target_user: '한국 학습자', visibility: 'private', assistance_disclosure: '질문형 AI 1회 사용', evidence_labels: ['BUILT', 'TESTED', 'DEBUGGED', 'EXPLAINED', 'MAINTAINED'], publishable: true, published: false };
  const fetchMock = vi.fn().mockImplementation((path: string, options?: RequestInit) => {
    if (path === '/api/portfolio' && !options?.method) return ok({ items: [] });
    if (path === '/api/portfolio') return ok(item);
    if (path === '/api/portfolio/p1/publish') return ok({ ...item, visibility: 'public', published: true });
    throw new Error(`unexpected ${path}`);
  });
  vi.stubGlobal('fetch', fetchMock);
  ...
  fireEvent.click(screen.getByRole('button', { name: '비공개 포트폴리오 만들기' }));

  expect(await screen.findByText('공개 범위: 비공개')).toBeInTheDocument();
  expect(screen.getByText('디버깅함')).toBeInTheDocument();
  expect(screen.getByText('유지보수함')).toBeInTheDocument();
  fireEvent.click(screen.getByRole('button', { name: '권리 확인 후 게시' }));
  expect(await screen.findByText('공개 범위: 공개')).toBeInTheDocument();
  const create = fetchMock.mock.calls.find(([path, options]) => path === '/api/portfolio' && options?.method === 'POST')!;
  expect(JSON.parse(create[1].body)).toMatchObject({ visibility: 'private', assistance_disclosure: '질문형 AI 1회 사용', idempotency_key: 'portfolio-key' });
});
