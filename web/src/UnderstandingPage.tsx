...
type Challenge = { id: string; prompt: Record<string, string>; state: string; predictions_committed?: boolean };
...
function UnderstandingPage() {
  ...
  useEffect(() => {
    fetch('/api/v1/understanding/challenges', { method: 'POST', credentials: 'include', headers, body: JSON.stringify({ workspace_id: workspaceId, kind: 'explanation_modification_transfer', idempotency_key: createKey.current }) })
      ...
      .then((body) => { setChallenge(body); setPredictionsCommitted(Boolean(body.predictions_committed)); }).catch((error) => setMessage(error.message ?? '이해 과제를 만들지 못했습니다.'));
  ...
  return <div className="learning-page"><PortalHeader /><main className="path-panel understanding-panel">
    ...
    {challenge && <section className="project-form">{Object.values(challenge.prompt).map((prompt) => <p key={prompt}>{prompt}</p>)}<p>현재 상태: {stateLabels[challenge.state.toLowerCase()] ?? challenge.state}</p>
    ...
    {result && <section className="evidence-card" aria-live="polite"><h2>{stateLabels[result.state.toLowerCase()] ?? result.state}</h2><p>도움 공개: {result.assistance_disclosure}</p><ul className="evidence-list">{result.evidence_labels.map((label) => <li key={label}>{evidenceLabels[label] ?? label}</li>)}</ul></section>}
