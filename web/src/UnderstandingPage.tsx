...
function UnderstandingPage() {
  ...
  const submit = async () => {
    ...
    const response = await fetch(`/api/v1/understanding/challenges/${challenge.id}/submit`, { method: 'POST', credentials: 'include', headers, body: JSON.stringify({ ...answers, idempotency_key: submitKeys.current[signature] }) });
  ...
  return <div className="learning-page"><PortalHeader /><main className="path-panel understanding-panel">
    ...
    {challenge && <section className="project-form">{Object.values(challenge.prompt).map((prompt) => <p key={prompt}>{prompt}</p>)}<p>현재 상태: {stateLabels[challenge.state] ?? challenge.state}</p>
      ...
      <label>독립 변형 실행 ID<input value={answers.modification_run_id} onChange={(event) => update('modification_run_id', event.target.value)} /></label>
      <label>전이 실행 ID<input value={answers.transfer_run_id} onChange={(event) => update('transfer_run_id', event.target.value)} /></label>
