...
type Policy = { version: string; title: string; required: boolean; consented: boolean };
...
function PoliciesPage() {
  ...
  const consent = async (event: FormEvent) => {
    ...
    if (!pending || checked !== pending.version) return;
  ...
  return <div className="learning-page"><PortalHeader /><main className="policy-page">
    ...
    <section className="policy-list" aria-label="정책 버전">
      {items.map((item) => <article key={item.version}><div><strong>{item.title}</strong><span>버전 {item.version}{item.required ? " · 필수" : ""}</span></div><b>{item.consented ? "동의 완료" : "동의 필요"}</b></article>)}
    </section>
    {pending && <form className="policy-consent" onSubmit={consent}>
      <label><input type="checkbox" checked={checked === pending.version} onChange={(event) => setChecked(event.target.checked ? pending.version : "")} /><span>{pending.version} 정책을 확인했습니다</span></label>
      <button type="submit" disabled={checked !== pending.version}>필수 정책에 동의</button>
    </form>}
