...
function GovernancePage() {
  ...
  const saveRights = (event: FormEvent) => { event.preventDefault(); act(`${base}/rights`, { basis: "original", evidence, commercial_use_allowed: commercial, redistribution_allowed: redistribution }, "권리 근거를 저장하고 기존 승인을 초기화했습니다."); };
  ...
  return <div className="learning-page"><PortalHeader /><main className="governance-page">
    ...
    <div className="governance-grid">
      {creator && <section><h2>권리 근거 기록</h2><form onSubmit={saveRights}><label><span>권리 근거</span><textarea minLength={3} maxLength={2000} value={evidence} onChange={(event) => setEvidence(event.target.value)} required /></label><label className="governance-check"><input type="checkbox" checked={commercial} onChange={(event) => setCommercial(event.target.checked)} /><span>상업 이용 허용</span></label><label className="governance-check"><input type="checkbox" checked={redistribution} onChange={(event) => setRedistribution(event.target.checked)} /><span>재배포 허용</span></label><button type="submit" disabled={!slug}>권리 근거 저장</button></form><button type="button" disabled={!slug} onClick={() => act(`${base}/publish`, {}, "게시 승인이 완료됐습니다.")}>분리 승인 확인 후 게시</button></section>}
