...
function ProviderControlsPage() {
  ...
  const validateProvider = async (provider: Provider) => { try { const response = await fetch(`/api/v1/content/providers/${provider.id}/validate`, { method: "POST", credentials: "include", headers: { "x-csrf-token": csrf() } }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error?.message ?? "제공자 설정을 검증하지 못했습니다."); setMessage("URL·모델·자격 증명 설정을 검증했습니다. 실제 실행 상태는 다음 호출에서 확인됩니다."); await load(); } catch (error) { setMessage(error instanceof Error ? error.message : "제공자 설정을 검증하지 못했습니다."); } };
  ...
  return <div className="learning-page"><PortalHeader /><main className="factory-page">
    ...
    <div className="factory-grid">
      ...
      <section><h2>예산과 상태</h2><form onSubmit={saveBudget}><label><span>전역 한도(마이크로 단위)</span><input type="number" min="0" max="1000000000000" value={limit} onChange={(event) => setLimit(Number(event.target.value))} /></label><button type="submit">예산 저장</button></form>
        ...
        <div className="factory-jobs">{providers.map((provider) => <article key={provider.id}><div><strong>{provider.name}</strong><span>{provider.enabled ? "활성" : "비활성"}</span></div><small>{provider.kind} · {provider.protocol === "anthropic" ? "Anthropic" : "OpenAI 호환"} · {provider.model} · 건당 {provider.cost_per_generation_microunits.toLocaleString()} · {!provider.credential_available ? "자격 증명 없음" : provider.health_status === "healthy" ? "최근 실제 호출 정상" : provider.health_status === "unhealthy" ? "최근 실제 호출 실패" : "키 참조 설정됨(실행 미확인)"} · 스트리밍/도구 미지원</small><div><button type="button" onClick={() => updateProvider(provider, { enabled: !provider.enabled })}>{provider.enabled ? "비활성화" : "활성화"}</button><button type="button" onClick={() => validateProvider(provider)}>설정 검증</button><select aria-label={`${provider.name} 대체 제공자`} value={provider.fallback_provider_id ?? ""} onChange={(event) => updateProvider(provider, { fallback_provider_id: event.target.value || undefined })}><option value="">대체 제공자 없음</option>{providers.filter((candidate) => candidate.id !== provider.id && candidate.enabled).map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.name}</option>)}</select></div></article>)}</div>
      </section>
