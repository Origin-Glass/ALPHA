...
function ProjectWorkspacePage() {
  ...
  return <div className="learning-page"><PortalHeader /><main className="path-panel project-learning-panel"><p className="eyebrow">프로젝트 작업 공간</p><h1>{project?.title ?? '프로젝트 불러오는 중'}</h1>{project && <><p>도움 정책: {project.assistance_policy} · 기술: {project.technology}</p><ol className="unit-list">{project.milestones.map((m) => <li className="unit-card" key={m.id}><div className="unit-position">{String(m.position).padStart(2, '0')}</div><div><h2>{m.title}</h2><p>{m.estimated_minutes}분 · 역량 {skillLabels[m.required_skill]} · 활동 {m.required_activity_slug}</p><button type="button" onClick={() => requestHelp(m)}>다음 도움 요청</button></div><span className="unit-state">{m.status}</span></li>)}</ol></>}{help && <p role="status" className="auth-message">{help}</p>}</main></div>;
}
